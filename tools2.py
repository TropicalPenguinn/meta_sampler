    ...
    with tqdm.tqdm(tfevent_files) as t:
        for tfevent_file in t:
            ...
            output_file = os.path.join(
                os.path.split(tfevent_file)[0], "test_reward2.csv"
            )
            ...
            for test_reward in ea.scalars.Items("score/episodic"):
                content.append(
                    [
...
def merge_csv(csv_files, root_dir, remove_zero=False):
    ...
    for rows in zip(*sorted_values):
        ...
        line += array[:, 1].tolist()
        content.append(line)
    output_path = os.path.join(root_dir, f"test_reward_seeds2.csv")
