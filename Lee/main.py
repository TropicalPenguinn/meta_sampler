...
i = 0
while True:
    if os.path.exists('run/{}/Lee/{}'.format(args.env_name,i)):
        i += 1
    else:
        break
writer = SummaryWriter('run/{}/Lee/{}'.format(args.env_name, i))
...
for i_episode in itertools.count(1):
    ...
    while not done:
        ...
        if previous_meta_state is not None:
            meta_memory.add(previous_meta_state,previous_meta_action,reward,meta_state,done)
        state = next_state
