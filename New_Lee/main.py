...
i = 0
while True:
    if os.path.exists('run/{}/New_Lee/{}'.format(args.env_name,i)):
        i += 1
    else:
        break
writer = SummaryWriter('run/{}/New_Lee/{}'.format(args.env_name, i))
...
scores_window2=deque(maxlen=20)
for i_episode in itertools.count(1):
    ...
    while not done:
        ...
        if previous_meta_state is not None:
            meta_memory.add(previous_meta_state,previous_meta_action,reward-np.mean(scores_window),meta_state,done)
        state = next_state
    ...
    if total_numsteps > args.num_steps:
        break
print("meta sampler percentage: {}".format(meta_action_num/args.num_steps))
env.close()
